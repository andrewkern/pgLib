...
void Yanggradient_bfgs (int n, const double x[], const int need[], double f0, 
...
void getgradient_bfgs(int npar, const double invec[], const int need_gradient[], 
     ...
     
{
  ...
  if (CENTRALMODE > 0)
    Yanggradient_bfgs(npar, invec, need_gradient, f0, outvec, func, 
...
void numeric_likelihood(const double *invec, double *outvec) {
  getgradient_bfgs(numpar0, invec, need0, outvec, func0, lowbound0, upbound0);
}
...
int test_mval = 0;
double test_factr = 0.0;
double test_pgtol = -1.0;
...
double findmax_bfgs(int numpars, double *invec, double (*fun)(const double x[]),
  ...
  if (test_mval > 0) m = test_mval;
  if (test_factr > 0) factr = test_factr;
  if (test_pgtol > -1.0) pgtol = test_pgtol;
  if (noisy > 0) printf("findmax_bfgs m=%i factr=%e pgtol=%e\n",
			m, factr, pgtol);
  if (dfun==NULL) {
